...
const Contact = () => {
  ...
  return (
    <section id="kontakt" className="py-24 max-w-6xl m-auto" style={{ backgroundColor: "var(--color-bg)" }}>
      <div className="container mx-auto px-6">
        <div className="grid lg:grid-cols-2 gap-16">
          ...
          <div>
            ...
            <div className="space-y-6">
              {contactInfo.map((item, index) => (
                ...
                >
                  <div className="w-12 h-12 rounded-xl flex items-center justify-center transition-all" style={{ backgroundColor: "var(--color-yellow-bg)" }}>
                    <item.icon className="w-5 h-5" style={{ color: "var(--color-yellow)" }} />
                  </div>

                  <div>
                    <div className="text-sm mb-1" style={{ color: "var(--color-text-secondary)" }}>{item.label}</div>
                    <div className="font-semibold whitespace-pre-line transition-colors group-hover:underline" style={{ color: "var(--color-text)" }}>
                      {item.value}
                    </div>
                  </div>
                </a>
          ...
          <div className="space-y-6">
            {/* Map */}
            <div className="rounded-2xl overflow-hidden border-2 h-72" style={{ backgroundColor: "var(--color-bg-elevated)", borderColor: "var(--color-border)" }}>
              <iframe
                ...
                referrerPolicy="no-referrer-when-downgrade" title="Lønbæks Køreskole"
              />
            </div>

            {/* Hours */}
            <div className="rounded-2xl border-2 p-6" style={{ backgroundColor: "var(--color-bg-elevated)", borderColor: "var(--color-border)" }}>
              ...
              <div className="space-y-4 text-sm">
                <div className="flex justify-between">
                  <span style={{ color: "var(--color-text-secondary)" }}>Teoriundervisning</span>
                  <span style={{ color: "var(--color-text)" }}>{t("contact.hours.theory")}</span>
                </div>
                <div className="flex justify-between">
                  <span style={{ color: "var(--color-text-secondary)" }}>Køretimer</span>
                  <span style={{ color: "var(--color-text)" }}>{t("contact.hours.driving")}</span>
                </div>
                <div className="flex justify-between">
                  <span style={{ color: "var(--color-text-secondary)" }}>Telefontid</span>
                  <span style={{ color: "var(--color-text)" }}>{t("contact.hours.phone")}</span>
                </div>
              </div>
            </div>
          </div>

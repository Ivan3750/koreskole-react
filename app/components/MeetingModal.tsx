...
const MeetingModal: React.FC<MeetingModalProps> = ({ onClose }) => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!formData.name || !formData.email) {
      setStatus("error");
      setMessage(!formData.name ? t("bookingForm.nameRequired") : t("bookingForm.emailRequired"));
      return;
    }
    try {
      const res = await fetch(`${BASE_URL}/book-meeting.php`, {
        method: "POST",
  ...
  return (
    <div className="fixed inset-0 bg-black bg-opacity-50 flex justify-center items-center z-50">
      <div className="bg-white rounded-2xl p-8 w-full max-w-md relative">
        ...
        <h2 className="text-2xl font-bold mb-2">{t("prices.meeting.modal.title")}</h2>
        <p className="text-sm text-gray-500 mb-6">{t("prices.meeting.modal.subtitle")}</p>
